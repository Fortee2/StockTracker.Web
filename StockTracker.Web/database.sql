...
ALTER TABLE `tickers` DROP COLUMN `close`;

ALTER TABLE `tickers` DROP COLUMN `in_portfolio`;

ALTER TABLE `tickers` DROP COLUMN `trend`;

CREATE TABLE `JobStatuses` (
    `Id` int NOT NULL AUTO_INCREMENT,
    `JobName` varchar(200) NOT NULL,
    `ActivityTime` datetime(6) NOT NULL,
    `ActivityDescription` varchar(500) NULL,
    CONSTRAINT `PK_JobStatuses` PRIMARY KEY (`Id`)
);

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20230401184303_create_jobstatus_table', '6.0.10');

COMMIT;

START TRANSACTION;

ALTER TABLE `JobStatuses` CHANGE `id` `Id` int NOT NULL DEFAULT 0;

ALTER TABLE `rsi` ADD `activity_date` date NOT NULL DEFAULT '0001-01-01 00:00:00.000000';

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20230422130136_ActivityDate_RSI', '6.0.10');
